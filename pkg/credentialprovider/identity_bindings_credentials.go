...
const (
	...
	// IdentityBindingsAPIServerIP is the fixed IP address where identity bindings endpoint is accessible
	// The SNI hostname should always resolve to this IP (typically the API server's cluster IP)
	IdentityBindingsAPIServerIP = "10.0.0.1"
)
...
// The transport uses a custom dialer that resolves the SNI name to the fixed API server IP
func createTransport(sniName string, caPool *x509.CertPool) *http.Transport {
	...
	transport.DialContext = func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		// Always connect to the fixed API server IP
		fixedAddr := net.JoinHostPort(IdentityBindingsAPIServerIP, port)
...
func (c *identityBindingsTokenCredential) GetToken(ctx context.Context, opts policy.TokenRequestOptions) (azcore.AccessToken, error) {
	...
	// Get client assertion token from request
	clientAssertion := c.req.ServiceAccountToken
	if clientAssertion == "" {
